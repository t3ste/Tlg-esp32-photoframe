...
- Agenda mode currently only fires from the deep-sleep timer-wake path (`main.c`'s `deep_sleep_wake_main()`) - if Deep Sleep is disabled (e.g. for Home Assistant / always-on use), Agenda Mode silently never runs, with no warning anywhere in the Web UI or docs. Needs a decision on whether/how to support it from the always-on `rotation_timer_task()` (`power_manager.c`) as well
...
- Investigate the ESP-IDF NVS partition possibly running out of free pages under heavy cumulative write load over a long-running device (config saves, per-fetch ETag updates, Telegram dedup-ID list, etc.) - the standard `nvs_flash_init()` boilerplate silently wipes the *entire* NVS partition (WiFi credentials included) via `nvs_flash_erase()` when it returns `ESP_ERR_NVS_NO_FREE_PAGES`/`ESP_ERR_NVS_NEW_VERSION_FOUND`, which would explain unexplained WiFi-reprovisioning-after-reflash incidents observed in the field without a flash tool ever having touched the NVS region directly. Not yet confirmed via a captured log of the actual erase happening; worth adding diagnostics (e.g. logging free-page count, or logging if `nvs_flash_erase()` is ever hit) and/or enlarging the 24KB NVS partition if confirmed
- A device that fell back to WiFi captive-portal mode was observed needing a second, separate reprovisioning shortly after the first succeeded - not root-caused (a weak signal, RSSI -71 dBm, was observed in one occurrence but not confirmed as the cause)
