...
- ~~Surface the next scheduled Agenda wake in the Web UI~~ — turned out to already exist: the Agenda tab's Schedule section reuses the same `RotationSchedule.vue` component as Auto-Rotate, which already computes and shows its own "Upcoming rotations" preview (next few run times) purely client-side from whichever cron rules are bound to it - no backend endpoint needed, and nothing missing here after all
...
- Agenda mode: `ICS_MAX_EVENTS` (24, shared by all five calendar sources A-E) was sized for Calendar A/B's narrow 1-3 day display window, where it's essentially unreachable in practice. The three manual sources (C/D/E) now expand against a much wider 30-day window to avoid re-parsing a large file every wake, which makes this cap meaningfully reachable for a densely-booked source. Consider raising it specifically for C/D/E (a separate, larger constant/type) rather than for all five sources uniformly, since A/B's own use case doesn't need it and a shared bump would cost extra PSRAM for every source unnecessarily
...
- The new Alarm Clock feature (see [docs/ALARMCLOCK_FEASIBILITY.md](docs/ALARMCLOCK_FEASIBILITY.md)) is gated behind its own independent build-time flag (`build.py --alarmclock`, off by default, entirely excluded from the binary otherwise) - Chimes and Climate are NOT gated this way. Their expensive hardware-driver code (the ES8311 register sequences in `components/board_hal/src/audio_chime.c`; the SHTC3/SHT40 I2C driver components) already only compiles in for boards that need it, via the same underlying Kconfig mechanism (each board's entry in `components/board_hal/Kconfig` `select`s the driver(s) it actually has) - so this isn't a functional gap. What's missing is an *independent* opt-in/out toggle the way Alarm Clock has: Chimes/Climate support is implicitly all-or-nothing per board (tied to hardware presence, not a separate user choice), and the lightweight feature-level dispatcher files (`main/chime.c`, `main/climate.c`, `main/climate_history.c`) still compile unconditionally into every board's firmware today, including the one board with neither speaker nor sensor at all (`xiao_ee02`) - a small, purely flash-size inefficiency (nothing breaks; `board_hal_has_speaker()`/`board_hal_get_temperature()` already fail closed there), not urgent, but worth revisiting for consistency with the Alarm Clock precedent if tightening the build matrix's flash usage becomes a priority
